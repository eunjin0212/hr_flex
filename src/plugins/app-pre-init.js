...
async function initApp(app, disableSignOn){
    ...
    if( ! disableSignOn ){
        ...
        if( authResponse.result ){ // SignOut 상태에서는 result==null 이다.
            window.logOnProfile = authResponse.result;
        }
    }
    ...
    app.config.globalProperties.$cdnImg=(s, postfix)=>{
        return window.__VUE_APP_CDN_PREFIX + s + (postfix||'');
    }
    app.config.globalProperties.$nasImg=(s, postfix)=>{
        return window.__VUE_APP_ORG_NAS_PREFIX + s + (postfix||'');
    };
    ...
    app.provide('cdnImg', app.config.globalProperties.$cdnImg);
    app.provide('orgImg', app.config.globalProperties.$nasImg);
