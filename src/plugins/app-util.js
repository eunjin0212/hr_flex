...
var ValidateUtil = {
    ...
    checkIsEmail(word) {
        return ( /^(?=.{1,200}@)[A-Za-z0-9_-]+(\.[A-Za-z0-9_-]+)*@[^-][A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*(\.[A-Za-z-]{2,})$/.test(word) );
    },
    checkPassword(word) {
        // 현재는 자릿수만 체크
    ...
    checkPhone(word) {
        if ( word == null) return false;
        return ( /^([0-9])([1-9])\d{7,10}$/.test(word) );
    },
    checkZipcode(word){
    ...
    checkNumberRange(start, end, word) {
        if (word.length < start) return false;
        if (word.length > end) return false;
        return true;
    }
};
